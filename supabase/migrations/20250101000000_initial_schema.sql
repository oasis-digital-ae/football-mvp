...
ALTER FUNCTION "public"."create_or_update_profile_atomic"("p_user_id" "uuid", "p_username" "text", "p_first_name" "text", "p_last_name" "text", "p_email" "text", "p_birthday" "date", "p_country" "text", "p_phone" "text", "p_reffered_by" "text") OWNER TO "postgres";
...
COMMENT ON FUNCTION "public"."process_match_result_atomic"("p_fixture_id" integer) IS 'Processes match results and updates market caps. Runs as SECURITY DEFINER to bypass RLS when inserting into total_ledger and transfers_ledger.';
...
COMMENT ON FUNCTION "public"."process_share_purchase_atomic"("p_user_id" "uuid", "p_team_id" integer, "p_shares" integer, "p_price_per_share" numeric, "p_total_amount" numeric) IS 'Fixed-point arithmetic: All values in cents (BIGINT). Uses exact database-calculated price (market_cap / total_shares) for total_invested to ensure perfect consistency with current price calculation, eliminating rounding errors in % change.';
...
ALTER FUNCTION "public"."process_share_sale_atomic"("p_user_id" "uuid", "p_team_id" integer, "p_shares" integer, "p_price_per_share" numeric, "p_total_amount" numeric) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."process_share_sale_atomic"("p_user_id" "uuid", "p_team_id" integer, "p_shares" integer, "p_price_per_share" numeric, "p_total_amount" numeric) IS 'CRITICAL: total_amount = ROUND(price_per_share, 2) * shares. Ensures exact price matching.';
