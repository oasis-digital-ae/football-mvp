-- Fix Transfer Amount Consistency
-- Ensure winner's gain exactly equals loser's loss (no rounding discrepancies)
-- This fixes the issue where Brighton gained $0.50 but Manchester City lost $0.49
...
BEGIN
  ...
  IF v_fixture.result = 'home_win' THEN
    ...
    -- Calculate transfer as 10% of loser's snapshot cap, rounded to 2 decimals
  ...
  -- CRITICAL: Use the SAME transfer_amount for both winner and loser to ensure exact match
  ...
  IF v_loser_after_cap < 10 THEN
    ...
    -- This ensures both winner and loser use the same adjusted amount
  ...
  -- Get CURRENT market caps for updating teams table (FOR UPDATE lock to prevent race conditions)
  ...
  -- UNIFIED CALCULATION: Use snapshot-based transfer amount for BOTH display and updates
  -- This ensures consistency - what you see in the ledger is exactly what was transferred
  -- Apply the same transfer_amount (calculated from snapshots) to current caps for updates
  ...
  IF v_loser_new_cap < 10 THEN
    ...
    -- Use CURRENT cap for this check (since we're updating current state)
    v_transfer_amount := ROUND(v_loser_current_cap - 10.00, 2);
    v_loser_new_cap := 10.00;
    v_winner_new_cap := ROUND(v_winner_current_cap + v_transfer_amount, 2);
    -- Update snapshot-based display values to match the adjusted transfer
    v_loser_after_cap := 10.00;
    v_winner_after_cap := ROUND(v_winner_before_cap + v_transfer_amount, 2);
  END IF;
  ...
  -- CRITICAL: Use the SAME v_transfer_amount for both winner and loser
  IF NOT EXISTS(
    SELECT 1 FROM total_ledger 
    WHERE trigger_event_id = p_fixture_id 
      AND team_id = v_winner_team_id
  ...
  -- CRITICAL: Use the SAME v_transfer_amount (negative for display purposes)
  IF NOT EXISTS(
    SELECT 1 FROM total_ledger 
    WHERE trigger_event_id = p_fixture_id 
      AND team_id = v_loser_team_id
...
COMMENT ON FUNCTION process_match_result_atomic IS 'Fixed shares model: Price = market_cap / total_shares (1000). Ensures winner gain exactly equals loser loss by using the same transfer_amount variable for both teams. Uses ROUND() to prevent floating point precision errors.';
