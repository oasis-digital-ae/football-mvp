-- Fix the trigger to properly find transfers without timing issues
-- The issue is that the trigger looks for transfers within 5 minutes,
-- but transfers might be created after team updates or the timing might be off

-- Drop the existing trigger
DROP TRIGGER IF EXISTS total_ledger_trigger ON teams;

-- Create a new trigger function that doesn't rely on timing
...
BEGIN
    ...
    IF v_price_impact != 0 OR v_shares_traded != 0 THEN
        ...
        -- Look for the most recent transfer for this team (no time restriction)
        -- This will find transfers created around the same time as the team update
        SELECT * INTO v_recent_transfer
        FROM transfers_ledger 
        WHERE (winner_team_id = NEW.id OR loser_team_id = NEW.id)
        AND applied_at >= NOW() - INTERVAL '1 hour'  -- Extended window to 1 hour
...
BEGIN
    ...
    -- Check what was created in the ledger
    RAISE NOTICE 'Latest ledger entry for team %:', v_test_team_id;
    FOR v_test_team_id IN 
        SELECT id FROM total_ledger 
        WHERE team_id = v_test_team_id 
        ORDER BY created_at DESC 
        LIMIT 1
    LOOP
        RAISE NOTICE '  Ledger entry created with ID: %', v_test_team_id;
    END LOOP;
END $$;
