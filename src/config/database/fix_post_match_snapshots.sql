-- =====================================================
-- FIX: Recreate snapshots with proper post-match market caps
-- =====================================================
-- The current snapshots all show $100 market cap because they were created
-- with pre-match values. We need to recreate them with post-match progression.
-- =====================================================

-- First, clean up the incorrect 2024 match snapshots again
DELETE FROM team_state_snapshots 
WHERE team_id = 28 
  AND snapshot_type = 'match_result' 
  AND effective_at < '2025-01-01'::timestamp;

-- Now recreate them with proper post-match market cap progression
DO $$
DECLARE
    match_record RECORD;
...
BEGIN
    ...
    LOOP
        ...
        -- IMPORTANT: Use POST-MATCH market caps for the snapshots
        IF winner_team_id IS NOT NULL THEN
            -- Winner snapshot (POST-MATCH state)
            INSERT INTO team_state_snapshots (
                ...
                'fixture',
                winner_post_cap, -- POST-MATCH market cap
                ...
                CASE 
                    ...
                    THEN winner_post_cap / (SELECT shares_outstanding FROM teams WHERE id = winner_team_id)
                    ELSE 20.00 
                END,
                'win',
                transfer_amount,
            ...
            -- Loser snapshot (POST-MATCH state)
            INSERT INTO team_state_snapshots (
                team_id,
                snapshot_type,
                trigger_event_id,
                ...
                match_record.fixture_id,
                'fixture',
                loser_post_cap, -- POST-MATCH market cap
                ...
                CASE 
                    ...
                    THEN loser_post_cap / (SELECT shares_outstanding FROM teams WHERE id = loser_team_id)
                    ELSE 20.00 
                END,
                'loss',
                -transfer_amount,
        ...
        ELSE
            ...
            INSERT INTO team_state_snapshots (
                ...
                match_record.fixture_id,
                'fixture',
                winner_post_cap, -- Use the reconstructed market cap
                ...
                CASE 
                    ...
                    THEN winner_post_cap / (SELECT shares_outstanding FROM teams WHERE id = match_record.home_team_id)
                    ELSE 20.00 
                END,
                'draw',
                0,
            ...
            INSERT INTO team_state_snapshots (
                ...
                match_record.fixture_id,
                'fixture',
                loser_post_cap, -- Use the reconstructed market cap
                ...
                CASE 
                    ...
                    THEN loser_post_cap / (SELECT shares_outstanding FROM teams WHERE id = match_record.away_team_id)
                    ELSE 20.00 
                END,
                'draw',
                0,
    ...
    RAISE NOTICE 'Finished recreating snapshots with correct POST-MATCH market cap progression';
END $$;

-- Verify the new snapshots
SELECT 
    'New Manchester United snapshots with POST-MATCH progression' as description,
    id,
    team_id,
    snapshot_type,
    match_result,
