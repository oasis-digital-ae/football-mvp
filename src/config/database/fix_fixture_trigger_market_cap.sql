-- Fix the fixture trigger to use correct market cap values
-- The trigger is currently writing incorrect market_cap_before/after values to total_ledger

-- Drop the existing trigger
DROP TRIGGER IF EXISTS fixture_result_trigger ON fixtures;

-- Create the corrected fixture trigger function
...
DO $$
DECLARE
    v_test_team_id INTEGER := 24; -- Everton
    v_ledger_entries INTEGER;
BEGIN
    -- Count existing ledger entries for the test team
    SELECT COUNT(*) INTO v_ledger_entries
    FROM total_ledger 
    WHERE team_id = v_test_team_id;
    
    RAISE NOTICE 'Team % has % ledger entries', v_test_team_id, v_ledger_entries;
    
    -- Show the progression
    RAISE NOTICE 'Current ledger progression:';
    DECLARE
        v_entry RECORD;
    BEGIN
        FOR v_entry IN 
            ...
            ORDER BY event_date
        LOOP
            RAISE NOTICE '  %: % | Market Cap: % → % (impact: %) | Share Price: % → %', 
                v_entry.ledger_type,
                v_entry.event_description,
                v_entry.market_cap_before,
                v_entry.market_cap_after,
                v_entry.price_impact,
                v_entry.share_price_before,
                v_entry.share_price_after;
        END LOOP;
    END;
END $$;
